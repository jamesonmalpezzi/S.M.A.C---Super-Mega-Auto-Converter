...
class TranscodeApp:
    def __init__(self, root):
        ...
        # Log area with fixed height and resizable
        log_frame = tk.Frame(root, bg="#f0f0f0", height=280)
        log_frame.pack(pady=20, padx=10, fill=tk.BOTH, expand=True)
        log_frame.pack_propagate(False)  # Prevent frame from resizing based on content
        self.log_text = scrolledtext.ScrolledText(log_frame, width=80, state='normal', font=("Helvetica", 10), fg="green", bg="black")
        self.log_text.pack(padx=10, fill=tk.BOTH, expand=True)
        self.log_text.insert(tk.END, "Let's convert some media files!\n")
